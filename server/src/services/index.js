const AtracoesTuristicasServices = require("./AtracoesTuristicasServices");
const EventosServices = require("./EventosServices");
const GuiasTuristicosServices = require("./GuiasTuristicosServices");
const HospedagensServices = require("./HospedagensServices");
const InformacoesUteisServices = require("./InformacoesUteisServices");
const RestaurantesServices = require("./RestaurantesServices");
const UsuariosServices = require("./UsuariosServices");

module.exports = {
  AtracoesTuristicasServices: AtracoesTuristicasServices,
  EventosServices: EventosServices,
  GuiasTuristicosServices: GuiasTuristicosServices,
  HospedagensServices: HospedagensServices,
  InformacoesUteisServices: InformacoesUteisServices,
  RestaurantesServices: RestaurantesServices,
  UsuariosServices: UsuariosServices,
};
