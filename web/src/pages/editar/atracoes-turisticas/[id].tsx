...
function AtracaoTuristica(): JSX.Element {
  ...
  const API = `${process.env.NEXT_PUBLIC_API_URL}/atracoes-turisticas/${params.id}`;
  ...
  const handleSubmit = async (formData: { [key: string]: string }) => {
    ...
    const endpoint = `${process.env.NEXT_PUBLIC_API_URL}/atracoes-turisticas/${params.id}`;
    const options = {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
    ...
    try {
      HttpClient(endpoint, options)
        ...
        .then(() => {
          router.push("/editar/atracoes-turisticas");
        });
    } catch (error) {
      console.log(error);
    ...
    <>
      ...
      <section className={comumStyles.mainContainer}>
        <div className={comumStyles.introSection}>
          <h1 className={comumStyles.introTitulo}>Editar Atração Turística</h1>
        ...
        <Formulario
          ...
          rotaBotaoCancelar="/editar/atracoes-turisticas"
...
export default withSessionHOC(AtracaoTuristica);
